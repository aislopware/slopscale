...
// TestSystemUpstreamsPastMagicDNS reads the files the agent reads by
// default on a gateway that accepts the tailnet's DNS without
// systemd-resolved: tailscaled wrote MagicDNS into resolv.conf and kept the
// real resolvers in its backup.
func TestSystemUpstreamsPastMagicDNS(t *testing.T) {
	dir := t.TempDir()
	magic := filepath.Join(dir, "resolv.conf")
	backup := filepath.Join(dir, "resolv.pre-tailscale-backup.conf")

	require.NoError(
		t,
		os.WriteFile(
			magic,
			[]byte("# resolv.conf(5) file generated by tailscale\nnameserver 100.100.100.100\n"),
			0o600,
		),
	)
	require.NoError(t, os.WriteFile(backup, []byte("nameserver 192.0.2.53\n"), 0o600))

	got, err := SystemUpstreams(filepath.Join(dir, "resolved.conf"), magic, backup)
	require.NoError(t, err)
