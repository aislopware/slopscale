...
| `actorKind`                            | How the caller authenticated: `local` (the unix socket, so the CLI on the server), `api_key`, `oauth`, `session` (a console sign-in), `node` (a machine reporting through the control protocol, such as `node.client.disconnect` when it logs out on purpose) or `system` (the server on its own). |
