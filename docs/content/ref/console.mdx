...
networks, routes, services, apps, DNS and relays. *Logs* holds the audit log and the [SSH session recordings](/ref/ssh-recording). *Administration* holds the settings, the keys and the
...
`/admin/tsconnect/`. Running `make wasm` alone builds only the WebAssembly
client. A binary built without `make web` still serves `/admin/`, with a page
