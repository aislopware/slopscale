...
the keys and, for everyone, the page to ask for temporary access; a member
sees their own machines and their own API keys. *Connectivity* holds networks, routes,
services, apps, DNS and relays. *Logs* holds the audit log and the
[SSH session recordings](/ref/ssh-recording). *Administration* holds the
settings and the integrations, and only a role that may read them sees it,
except _Settings → Sessions_, which every signed-in user has for their own sign-ins. An item with several parts, such as _Access controls_, _DNS_,
...
- **Settings**: a page each for the tailnet switches
  ...
  _Switch off_), the console [sessions](#sessions) with _End_ and _Sign out
