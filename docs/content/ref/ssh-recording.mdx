...
description: "Record Tailscale SSH sessions as asciinema files with the embedded recorder or tsrecorder, enforce recording per rule, and list or download recordings."
---
...
Every failure the client reports lands in the [audit log](/ref/audit) as
...
`sshRecordingFailed` [webhook](/ref/webhooks) event.
