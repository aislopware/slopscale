...
description: "Let machines get Let's Encrypt certificates for their MagicDNS names, with slopscale publishing DNS-01 challenges via Cloudflare, RFC 2136 or a command."
---
...
anything, and the machines stay reachable over the tailnet alone, unless
[Funnel](./funnel) opens one of them to the internet.
...
The machine accepts the challenge the moment Slopscale answers, and
Let's Encrypt looks the record up right away, but a provider's API
takes a record some seconds before its nameservers serve it. So
Slopscale answers only once every authoritative nameserver of the zone
serves the record, giving up after `propagation_timeout`, a minute by
default; `0` answers as soon as the provider took the record.
...
lands in the [audit log](/ref/audit) as `node.cert_challenge` on the
machine.
