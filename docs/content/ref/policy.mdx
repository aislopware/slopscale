---
title: "Policy"
description: "Write a Tailscale-style policy file with grants or ACLs: allow-all and deny-all starting points, supported autogroups, node attributes and IP pools."
---
...
its path in the `policy.path` key in the [configuration file](/ref/configuration).
...
- See our documentation on [subnet routers](/ref/routes#subnet-router) and [exit nodes](/ref/routes#exit-node) to learn how
...
- [Device postures](https://tailscale.com/docs/features/device-posture) are supported through `postures`,
  `srcPosture` and `defaultSrcPosture`; see [Device trust](/ref/device-trust#postures-in-the-policy-file). Postures in the
...
- A [service](/ref/services) is a destination, `svc:web`, and
  `autoApprovers.services` names who may host one.
...
Allows access to the internet through [exit nodes](/ref/routes#exit-node). Can only be used in policy destinations.
...
[role](/ref/roles). Usable wherever `autogroup:member` is.
...
[shared](/ref/sharing) with. Can only be used in policy sources. The destination
...
- `suggest-exit-node`, `suggest-exit-node-ui`: [Automatic exit node selection](/ref/routes#suggested-exit-nodes).
  Slopscale stamps `suggest-exit-node` on approved exit nodes on its own, so the policy only needs these to narrow or
  widen what the client is offered.
- `nextdns:<profile>`, `nextdns:no-device-info`: [NextDNS integration](https://tailscale.com/docs/integrations/nextdns).
  Be sure to set NextDNS as global resolver in the [configuration](/ref/configuration).
...
- `disable-relay-server`: keep the node from acting as a [peer relay](/ref/networks#peer-relays), whatever its own
  `--relay-server-port` setting says.
...
[auto approver](/ref/routes) for its tag or by hand.
...
[^1]: Slopscale also allows to store the policy in the database. This is typically only required in case the
    [admin console](/ref/console) is used.
