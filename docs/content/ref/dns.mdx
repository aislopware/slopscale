---
title: "DNS"
description: "Change nameservers, split DNS, search domains and extra records at runtime, keep resolvers while an exit node is in use, and split DNS per group."
---

Slopscale supports [most DNS features](/about/features) from Tailscale. DNS related settings can be configured
within the `dns` section of the [configuration file](/ref/configuration), and most of them can be changed while the
...
`/api/v2/tailnet/-/dns/nameservers`, `/dns/preferences`, `/dns/searchpaths` and `/dns/split-dns` endpoints, plus
`/dns/configuration`, which reads and replaces every setting in one call the way `PUT /api/v1/dns` does, so the
...
change is pushed to the clients at once and logged in the [audit log](/ref/audit) as `dns.set` or `dns.reset`.
...
[groups](/ref/access-control) only: a rule names the zones, the resolvers that answer for them and the groups that
...
in the [configuration file](/ref/configuration) or from a JSON file that Slopscale continuously watches for changes:

- Use the `dns.extra_records` option in the [configuration file](/ref/configuration) for entries that are static and
...
- For dynamic DNS records that may be added, updated or removed while Slopscale is running or DNS records that are
  generated by scripts the option `dns.extra_records_path` in the [configuration file](/ref/configuration) is useful.
...
1. Configure extra DNS records using one of the available configuration options:
    ...
    <Tabs>
    <Tab title="Static entries, via dns.extra_records">
    ...
    <Tab title="Dynamic entries, via dns.extra_records_path">
    ...
    - The `dns.extra_records_path` option in the [configuration file](/ref/configuration) needs to reference the
    ...
    - Records are checked like those set at runtime: a file with an invalid name or a value that does not match
      its type stops Slopscale from starting, and an invalid edit while it runs is logged and the records already
      served stay in place.
    :::
