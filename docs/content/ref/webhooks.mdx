...
resolves to when it delivers; see [outbound requests](/ref/configuration#settings-that-live-only-in-the-file) for
...
| `nodeSuspended`         | An administrator suspended a machine; see [Device trust](/ref/device-trust).             |
...
| `accessRequestCreated`  | A user asked to join a group for a while; see [Temporary access](/ref/temporary-access). |
...
| `sshRecordingFailed`    | An SSH session could not be recorded; see [SSH session recording](/ref/ssh-recording).   |
...
addresses, and a person listed both ways is mailed once. An endpoint
that resolves to nobody records the delivery as failed rather than
retrying it.
