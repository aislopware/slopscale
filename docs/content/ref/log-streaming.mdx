---
title: "Log streaming"
description: "Ship the audit log to Splunk, Elastic, Datadog, Axiom, Loki or any HTTP collector as it is written, and how batches are retried or dropped."
---

Log streaming ships the [audit log](/ref/audit) to a SIEM or log store as it
...
The URL is subject to the same [outbound request rules](/ref/configuration#settings-that-live-only-in-the-file)
as a webhook: no loopback or link-local sink. The token is stored and never listed again; an update that leaves it empty
...
`configuration`: the [traffic monitor](/ref/traffic) is not streamed; read
it through its API.
