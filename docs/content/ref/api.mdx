...
| `preauthorized` | `false`                     | While [device approval](/ref/approval) is on, a node joined with `false` waits for approval and one joined with `true` does not. |
...
| `tailscale_logstream_configuration` | Partial   | `configuration` streams the [audit log](/ref/audit). `network` is refused with `404`: network flow logs are not streamed, they are read from `GET /api/v2/tailnet/-/logging/network` (see the [traffic monitor](/ref/traffic#tailscale-compatible-api)). Destinations are `http`, `splunk`, `elastic`, `datadog`, `axiom` and `loki`; S3 and Cloud Storage are refused by name. |
