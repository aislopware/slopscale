...
func lookupPR(ctx context.Context, r *repo, slug, branch string) (*prState, error) {
	...
	if len(prs) == 0 {
		//nolint:nilnil // absence is the normal first-run case
		return nil, nil
	}
...
func updateOpen(ctx context.Context, r *repo, opt publishOptions, pr *prState, body string, now marker) error {
	...
	// The title follows the areas that landed, because it becomes the squash
	// commit's subject and decides whether the merge is a release.
	_, err = r.run(ctx, "gh", "pr", "edit", strconv.Itoa(pr.Number), "--repo", opt.Slug,
	...
	_, err = r.run(ctx, "gh", "pr", "comment", strconv.Itoa(pr.Number), "--repo", opt.Slug,
		"--body", "The set of applied areas changed since the last run; see the updated description.")
	if err != nil {
		return err
	}
