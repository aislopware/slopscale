...
func (r *repo) runIn(ctx context.Context, dir string, argv ...string) (string, error) {
	...
	//nolint:gosec // argv is built from repo state, not user input
	...
	cmd.Env = append(os.Environ(), "GOWORK=off", "NIX_CONFIG="+nixConfig())
...
// nixConfig extends the caller's NIX_CONFIG. The dirty-tree warning is noise on
// a tree the bump is editing by design, and nix fetches flake inputs through
// the GitHub API, whose unauthenticated budget a lock update exhausts.
func nixConfig() string {
	lines := []string{os.Getenv("NIX_CONFIG"), "warn-dirty = false"}

	if token := githubToken(); token != "" {
		lines = append(lines, "access-tokens = github.com="+token)
	}
