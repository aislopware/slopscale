...
func TestNetInfoPreservationInRegistrationFlow(t *testing.T) {
	...
	t.Run("registration_flow_bug_reproduction", func(t *testing.T) {
		...
		assert.Equal(
			t,
			tailcfg.DERPRegionID(5),
			correctResult.PreferredDERP,
			"Should preserve the DERP region from existing node",
		)
	})
		...
		for _, tt := range []struct {
			...
			t.Run(name, func(t *testing.T) {
				...
				for _, derp := range []tailcfg.DERPRegionID{tt.firstDERP, tt.lastDERP} {
					...
					wg.Go(func() {
						c, updateErr := s.UpdateNodeFromMapRequest(nodeID, request)
						results <- result{change: c, err: updateErr}
					})
...
// resend the whole node to every peer. DeviceModel is a posture input, so
// the policy manager is refreshed; with no posture in the policy that is
// not a change either.
...
func TestPeerHostinfoEqual(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "netinfo udp",
			mutate: func(hi *tailcfg.Hostinfo) { hi.NetInfo.WorkingUDP = opt.NewBool(false) },
			want:   true,
		},
		...
		{
			name:   "ssh host keys",
			mutate: func(hi *tailcfg.Hostinfo) { hi.SSH_HostKeys = []string{"ssh-ed25519 AAAA"} },
			want:   false,
		},
		{
			name:   "location",
			mutate: func(hi *tailcfg.Hostinfo) { hi.Location = &tailcfg.Location{Priority: 5} },
			want:   false,
		},
		{
			name:   "app connector",
			mutate: func(hi *tailcfg.Hostinfo) { hi.AppConnector = opt.NewBool(true) },
			want:   false,
		},
		{
			name: "routes",
			mutate: func(hi *tailcfg.Hostinfo) {
				hi.RoutableIPs = []netip.Prefix{netip.MustParsePrefix("10.0.0.0/24")}
			},
			want: false,
		},
	}
...
func TestHostinfoDERP(t *testing.T) {
	...
	require.Equal(t, tailcfg.DERPRegionID(5),
		hostinfoDERP(&tailcfg.Hostinfo{NetInfo: &tailcfg.NetInfo{PreferredDERP: 5}}))
}
