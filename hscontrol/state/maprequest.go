...
type mapRequestDelta struct {
	...
	// out (a newly-added useful non-STUN endpoint). Storage still
	...
	// capChanged reports whether the client's capability version moved.
	// It rides as a patch so peers learn that an upgraded client now
	// speaks peer relay, which magicsock gates on [tailcfg.Node.Cap].
	capChanged bool

	// attestationMoved reports whether the hardware attestation verdict
...
// policyInputsChanged reports whether something the policy manager reads
// may have moved, so the O(n) SetNodes scan is worth running.
func (d mapRequestDelta) policyInputsChanged() bool {
	return d.peerHostinfoChanged || d.routesChanged || d.postureChanged || d.attestationMoved
}
