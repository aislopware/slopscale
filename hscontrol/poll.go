...
func (m *mapSession) stopFromBatcher() {
	if m.cancelChClosed.CompareAndSwap(false, true) {
		...
		// A channel signal cannot interrupt a response write that is blocked
		// on HTTP/2 flow control. Expire the stream's write deadline as well
		// so a client that stopped reading cannot keep the map session, and
		// therefore server shutdown, alive indefinitely.
