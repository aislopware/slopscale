...
// initialMapSendTimeout bounds how long [Batcher.AddNode] waits for the
// initial map to be accepted by the connection channel.
...
// mapResponseCounters holds one counter per [change.Change] category with its
// label resolved once. Every node the change reaches increments one, and
// [prometheus.CounterVec.WithLabelValues] takes the vector's lock and hashes
// the label on each call, so the counters are resolved at startup instead.
var mapResponseCounters = func() map[string]prometheus.Counter {
	counters := make(map[string]prometheus.Counter, len(change.Types))
	for _, t := range change.Types {
		counters[t] = mapResponseGenerated.WithLabelValues(t)
	}

	return counters
}()

// incMapResponseGenerated counts one generated map response of category t on
// its pre-resolved counter, falling back to a label lookup for a category
// missing from [change.Types].
func incMapResponseGenerated(t string) {
	if c, ok := mapResponseCounters[t]; ok {
		c.Inc()

		return
	}

	mapResponseGenerated.WithLabelValues(t).Inc()
}
...
// generateMapResponse generates a [tailcfg.MapResponse] for the given
// [types.NodeID] based on the provided [change.Change].
func generateMapResponse(nc nodeConnection, mapper *mapper, r change.Change) (*tailcfg.MapResponse, error) {
	...
	case r.RequiresRuntimePeerComputation:
		...
		// so the node sees its updated self info along with new packet filters, or
		// when the change asks for it on every node (approval flips
		// MachineAuthorized on the self node of everyone it admits).
		mapResp, err = mapper.policyChangeResponse(
			nodeID, version, removedPeers, currentPeers, isSelfUpdate || r.IncludeSelf,
		)
	case isSelfUpdate:
...
func handleNodeChange(nc nodeConnection, mapper *mapper, r change.Change) error {
	...
	log.Debug().
		...
		Str(zf.Reason, r.Reason).
		Msg("node change processing started")
...
type Batcher struct {
	...
	// nodes is keyed by node ID. Callbacks passed to its Compute must not
	// panic: xsync holds the bucket lock across the callback and does not
	// release it on panic (puzpuzpuz/xsync#208), which would wedge every
	// later operation on that bucket.
...
func NewBatcherAndMapper(cfg *types.Config, st *state.State) *Batcher {
	m := newMapper(cfg, st)
...
func (b *Batcher) AddNode(
	...
	case <-time.After(initialMapSendTimeout):
		...
		nlog.Debug().Caller().Dur("timeout.duration", initialMapSendTimeout).
...
// pending entries or in-flight work. A PeersRemoved or DeletedNodes change
// is not empty, so deletion cleanup in addToBatch still runs.
...
func (b *Batcher) Close() {
	...
	b.nodes.Range(func(_ types.NodeID, conn *multiChannelNodeConn) bool {
		if conn == nil {
			return true
		}
...
// processSyncWork generates the map response for a synchronous work item
// and delivers it on w.resultCh. It returns false when the batcher shut
// down before the result could be delivered, in which case the worker
// must exit.
func (b *Batcher) processSyncWork(wlog *zerolog.Logger, w work) bool {
...
// processAsyncWork applies all bundled changes of an asynchronous work
// item to the node's connections sequentially.
...
func (b *Batcher) processAsyncWork(wlog *zerolog.Logger, w work) {
	nc, exists := b.nodes.Load(w.nodeID)
	if !exists || nc == nil {
		return
	}
...
func (b *Batcher) addToBatch(changes ...change.Change) {
	...
	// [change.Change.PeersRemoved] remains only the protocol delta sent to
	// clients when peers disappear from their view, so a policy change that
	// merely narrows peer visibility cannot kill a live node's long poll.
	//
	...
	// TCP session broke (aislopware/slopscale#3410).
