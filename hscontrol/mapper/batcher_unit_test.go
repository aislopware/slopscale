...
// TestAddWorkRemovalsNotTreatedAsEmpty ensures the empty filter does not
// swallow a PeersRemoved or DeletedNodes change: deletion cleanup must
// still run, and surviving recipients must still see the removal.
func TestAddWorkRemovalsNotTreatedAsEmpty(t *testing.T) {
	t.Parallel()
	...
	require.False(t, removal.IsEmpty(), "a DeletedNodes change is not empty")
	require.False(t, change.Change{PeersRemoved: []types.NodeID{4}}.IsEmpty(),
		"a PeersRemoved change is not empty")
