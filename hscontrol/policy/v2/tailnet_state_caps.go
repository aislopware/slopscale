...
// [tailcfg.NodeAttrDNSSubdomainResolve]), each under its own condition. It returns nil when none applies, matching
...
var unmodelledTailnetStateCaps = []nodecap.Cap{
	...
	// Slopscale stamps it on every connector node of an app (see
	// stampAppConnectorCaps); the captures carry no apps.
