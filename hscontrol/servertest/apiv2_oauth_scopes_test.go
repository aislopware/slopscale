...
// tskey-client- secret against slopscale: the client parses the ?attributes,
...
// and what slopscale must do with it.
func TestAPIv2OAuthTailscaleClientAuthKey(t *testing.T) {
	...
	toTS := func(secret string) string {
		return types.TailscaleOAuthClientPrefix + strings.TrimPrefix(secret, types.OAuthClientPrefix)
	}

	secret, client := newClient()
	tsSecret := toTS(secret)
	wrongSecret := types.TailscaleOAuthClientPrefix + client.ClientID + "-" + strings.Repeat("0", 64)
	...
	tests := []struct {
		...
		{
			// A key minted with preauthorized=false holds the node for approval
			// only while device approval is on, which it is not here.
		...
		},
		{
			...
			// yields "//api/v2/oauth/token", which slopscale does not route.
		...
		},
		{
			...
			// client rejects anything else before contacting slopscale.
