	...
	for _, scheduled := range []bool{false, true} {
		t.Run(fmt.Sprintf("scheduled=%v", scheduled), func(t *testing.T) {
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				...
				"expiry must take the node offline without ending its control session")
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				...
				"restoring expiry must recover both clients without another login or poll")
		})
	}
}
	...
	for _, scheduled := range []bool{false, true} {
		t.Run(fmt.Sprintf("scheduled=%v", scheduled), func(t *testing.T) {
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				...
				"expiry must move the route to the standby while preserving control connectivity")
