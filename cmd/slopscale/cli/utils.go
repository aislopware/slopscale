...
const (
	SlopscaleDateTimeFormat = "2006-01-02 15:04:05"
...
var (
	errAPIKeyNotSet     = errors.New("SLOPSCALE_CLI_API_KEY environment variable needs to be set")
...
func newSlopscaleServerWithConfig() (*hscontrol.Slopscale, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("creating new slopscale: %w", err)
	}
...
func clientRunE(
	...
	return func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			return fmt.Errorf("connecting to slopscale: %w", err)
		}
		defer cancel()
...
// newSlopscaleCLIWithConfig builds an HTTP client for the Slopscale v1 API.
//
...
func newSlopscaleCLIWithConfig() (context.Context, *clientv1.ClientWithResponses, context.CancelFunc, error) {
	...
	if address == "" {
		log.Debug().
			...
			Msgf("SLOPSCALE_CLI_ADDRESS environment is not set, connecting to unix socket.")
...
func newSocketClient(socketPath string) (*clientv1.ClientWithResponses, error) {
	...
	if err != nil {
		if os.IsPermission(err) {
			return nil, fmt.Errorf(
				"unable to read/write to slopscale socket %q, do you have the correct permissions? %w",
				socketPath,
				err,
			)
		}
...
// Socket dial retry backoff bounds; see dialSlopscaleSocket.
const (
	socketDialInitialInterval = 50 * time.Millisecond
	socketDialMaxInterval     = 1 * time.Second
)

// dialSlopscaleSocket connects to the unix socket, retrying until it appears or
...
func dialSlopscaleSocket(ctx context.Context, socketPath string) (net.Conn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dialing slopscale socket %s: %w", socketPath, err)
	}
...
// newRemoteClient builds an API client for a remote Slopscale over HTTPS,
