...
func derpFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("region-code", "slopscale", "")
...
func sampleDERP() clientv1.DERP {
	...
	return clientv1.DERP{
		Overridden:     false,
		Paths:          []string{"/etc/slopscale/derp.yaml"},
		RelayAvailable: true,
		RelayRunning:   true,
		StunAddr:       "[::]:3478",
		ServerUrl:      "https://slopscale.example",
		...
		Regions: []clientv1.DERPMapRegion{
			...
			{Id: 999, Code: "slopscale", Name: "Slopscale Embedded DERP", Nodes: 1, Source: clientv1.Embedded},
		},
	}
}
	...
	cases := []commandCase{
		{
			...
			wantIn: []string{
				...
				"Map files: /etc/slopscale/derp.yaml",
				"Auto update: every 3h0m0s",
				"Last fetch: 2026-09-08T10:00:00Z",
				"Embedded relay: running at https://slopscale.example as region 999 (slopscale), STUN on [::]:3478",
				"Verify clients: on",
				"IPv4: 198.51.100.1",
				"Relays you run:",
				"20 sgp (Singapore)",
		...
		},
		{
			...
			wantIn: []string{
				"Source: set through the API (reset with `slopscale derp reset`)",
