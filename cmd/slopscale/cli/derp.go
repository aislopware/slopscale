...
func init() {
	...
	setDERPCmd.Flags().String("region-code", "slopscale", "Short code of the embedded relay's region")
...
managed with "slopscale derp relay".`,
	RunE: clientRunE(
		func(ctx context.Context, client *clientv1.ClientWithResponses, cmd *cobra.Command, _ []string) error {
...
func printDERPHuman(d *clientv1.DERP, prefix string) {
	...
	if d.Overridden {
		fmt.Println("Source: set through the API (reset with `slopscale derp reset`)")
	} else {
