...
func init() {
	...
	rootCmd.PersistentFlags().
		StringVarP(&cfgFile, "config", "c", "", "config file (default is /etc/slopscale/config.yaml)")
	rootCmd.PersistentFlags().
...
func initConfig() {
	if cfgFile == "" {
		cfgFile = os.Getenv("SLOPSCALE_CONFIG")
	}
	...
	if !disableUpdateCheck && !machineOutput {
		...
		if (runtime.GOOS == "linux" || runtime.GOOS == "darwin") &&
			...
			if err == nil && isOutdated(versionInfo.Version, newest) {
				log.Warn().Msgf(
					"An updated version of Slopscale has been found (%s vs. your current %s). "+
						"Check it out https://github.com/aislopware/slopscale/releases\n",
					newest,
					versionInfo.Version,
				)
			}
...
const (
	releasesURL          = "https://api.github.com/repos/aislopware/slopscale/releases?per_page=30"
...
var rootCmd = &cobra.Command{
	Use:   "slopscale",
	Short: "slopscale, an open source Tailscale control server",
	Long: `
slopscale is an open source implementation of the Tailscale control server

https://github.com/aislopware/slopscale`,
	SilenceErrors: true,
	SilenceUsage:  true,
}
