	...
	RunE: clientRunE(
		func(ctx context.Context, client *clientv1.ClientWithResponses, cmd *cobra.Command, _ []string) error {
			...
			if !confirmAction(cmd, fmt.Sprintf(
				"Do you want to remove the user %q (%s), its pre-auth keys and OAuth clients? "+
					"Nodes owned by the user must be deleted or moved first.",
				user.Name, user.Id,
			)) {
				...
				for _, user := range users {
					rows = append(
						...
						[]string{
							...
							user.CreatedAt.Format(SlopscaleDateTimeFormat),
						},
					)
				}
