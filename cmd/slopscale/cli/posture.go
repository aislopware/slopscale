...
answer. Needs the posture identity setting (slopscale settings set
...
the attribute at that time, which is how a temporary grant is made:
  ...
  slopscale nodes posture set -i 7 custom:oncall=true --expiry 8h`,
...
func printPosture(cmd *cobra.Command, posture *clientv1.NodePosture) error {
	return printListOutput(cmd, posture, func() error {
		...
		case posture.Identity == nil && !posture.IdentityCollectionOn:
			rows = append(rows, []string{"(identity)", "collection off; see slopscale settings set --posture-identity"})
		...
		case posture.Identity.Disabled:
			rows = append(rows, []string{
				...
				"client has posture checking off, asked " + posture.Identity.CollectedAt.Format(
					SlopscaleDateTimeFormat,
				),
			})
		default:
			rows = append(
				rows,
				[]string{"(identity)", "collected " + posture.Identity.CollectedAt.Format(SlopscaleDateTimeFormat)},
			)
		}

		for _, c := range posture.Custom {
			if c.ExpiresAt != nil {
				rows = append(rows, []string{c.Key + " expires", c.ExpiresAt.Format(SlopscaleDateTimeFormat)})
			}
		}
