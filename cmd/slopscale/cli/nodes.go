...
var registerNodeCmd = &cobra.Command{
	...
	Deprecated: "use 'slopscale auth register --auth-id <id> --user <user>' instead",
	RunE: clientRunE(
...
func nodesToPtables(nodes []clientv1.Node) (pterm.TableData, error) {
	...
	for _, node := range nodes {
		...
		if node.LastSeen != nil {
			lastSeenTime = node.LastSeen.Format(SlopscaleDateTimeFormat)
		}

		expiryTime := "N/A"
		if node.Expiry != nil {
			expiryTime = node.Expiry.Format(SlopscaleDateTimeFormat)
		}
