...
// TestDialSlopscaleSocketRetriesUntilPresent proves the CLI socket dialer
...
func TestDialSlopscaleSocketRetriesUntilPresent(t *testing.T) {
	...
	go func() {
		conn, err := dialSlopscaleSocket(ctx, sock)
...
// TestDialSlopscaleSocketRespectsDeadline proves the retry is bounded by the
// context: when the socket never appears, the dialer returns an error around the
// deadline instead of hanging.
func TestDialSlopscaleSocketRespectsDeadline(t *testing.T) {
