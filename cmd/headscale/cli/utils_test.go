...
func TestHasMachineOutputFlag(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "no output flag", args: []string{"headscale", "nodes", "list"}, want: false},
		{name: "json", args: []string{"headscale", "nodes", "list", "-o", "json"}, want: true},
		{name: "json-line", args: []string{"headscale", "--output", "json-line", "nodes", "list"}, want: true},
		{name: "yaml", args: []string{"headscale", "nodes", "list", "yaml"}, want: true},
	}
...
func TestNewHeadscaleCLIWithConfig(t *testing.T) {
	...
	t.Run("remote address without an api key is rejected", func(t *testing.T) {
		...
		_, client, _, err := newHeadscaleCLIWithConfig()
		...
		require.ErrorIs(t, err, errAPIKeyNotSet)
		require.ErrorContains(t, err, "connecting to headscale")
	})
...
func TestNewHeadscaleServerWithConfigRejectsEmptyConfig(t *testing.T) {
	viper.Reset()
