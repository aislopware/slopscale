...
- The embedded relay can run without STUN: `derp.server.stun_enabled: false` in the config file, `slopscale derp set --stun-enabled=false`, `stunEnabled: false` on `PUT /api/v1/derp` or the switch on the console's _Relays_ page publish the region without a STUN port and close the listener; Tailscale's relays still answer STUN. Use it when the relay sits on the machines' own network and they reach its public address through their router: the relay then sees the router's private address as each machine's source and says so in its STUN reply, which disagrees with Tailscale's STUN servers, so every machine on that network reported a hard NAT (`MappingVariesByDestIP: true`) and advertised the router's private address as its public endpoint. A request that leaves `stunEnabled` out keeps STUN on
- The embedded relay's STUN reply names an IPv4 client by its IPv4 address again when the listener is a dual-stack socket, which reported it as a v4-mapped IPv6 address
