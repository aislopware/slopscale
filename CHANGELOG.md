...
- A user's display name, email and profile picture can be changed after creation with `headscale users set` or `PATCH /api/v1/user/{id}`, and the clients show the new profile on their next map update. Renaming a user reaches the clients the same way, where before the map response kept the old profile until a restart [#2166](https://github.com/juanfont/headscale/issues/2166)
