...
- Peers now see each machine's capability version (`Cap` on the peer entry), as Tailscale's control plane sends it, and an upgrade reaches them as a patch. Clients gate peer relay and other newer paths on the peer's version and treated the missing value as an old client that could not use them
- A machine that asks for a map without streaming (`tailscale debug netmap`, `tsnet` programs and the browser client on their first request) now gets a full map response instead of an empty stream. A register request with a malformed body is refused with 400 before it can consume a pre-auth key, and a client below the supported capability version is refused before the request has any effect
...
- The `tailnet.maxKeyDuration` node capability carries the tailnet's key expiry in seconds when it is set, so `tailscale status` and the clients' key-expiry warnings use the server's value instead of the 180-day default
- `/machine/audit-log`, which a client posts when it disconnects on purpose (`tailscale logout`, `tailscale down`), is recorded in the audit log as action `node.client.disconnect` with actor kind `node`; the server answered 501 before. `/machine/feature/query`, behind `tailscale serve` on a machine without the capability, now tells the operator which node attribute to grant and links to the policy page instead of failing; Funnel is answered as unavailable because the policy engine does not accept its attribute
